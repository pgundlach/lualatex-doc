...
%
% Written by Manuel Pgouri-Gonnard <mpg@elzevir.fr>, 2010.
%
...
\author{Manuel Pgouri-Gonnard \email{mpg@elzevir.fr}}
\date{\today}
...
\pk{fontspec}. To be precised, \pf{euenc} merely declares the encoding, but
...
It doesn't work, however, if \luatex is invoked as lualatex'\inputname',
as opposed to the more usual lualatexname. To work around this
limitation of the workaround included in the format, specifying a jobname
explicitly, as in lualatexjobname=name '\inputname'. Or even better, just
...
\para{codes}
The engine itself does not set \catcodes, \lccodes, etc. for non-ASCII
characters. Correct \lccodes in particular are essential for hyphenation to
